...
def relu_backward(dout, cache):
    ...
    out =  np.maximum(0,x)
    out[out>0]=1 #local gradient 
    dx = out * dout #multiply local gradient with upstream gradient
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        sample_mean = np.mean(x,axis=0)                #minibatch mean per feature
        sample_var = np.var(x,axis=0)      #minibatch variance per feature
        x_norm = (x-sample_mean.T)/np.sqrt(sample_var.T+eps) #normalize
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    m=dout.shape[0] #size of minibatch
    ...
    dx = dx_norm/np.sqrt(cache['sample_var'] + cache['eps']) + dsample_var * 2*(cache['x']-cache['sample_mean'])/m + dsample_mean/m
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x, xhat, mean, var, gamma, beta, eps, G = cache
    m = C//G*H*W    
    ...
    dxhat = dout * gamma
    dxhatT = np.reshape(dxhat, (N*G, C//G*H*W)).T
    dvar = np.sum(dxhatT * (xT - mean), axis=0) * (-0.5) * np.power((var + eps), -1.5)
    dmean = np.sum(dxhatT, axis=0) * -1 / np.sqrt(var+eps) + dvar / m * np.sum(-2*(xT-mean), axis=0)
    dx = dxhatT / np.sqrt(var+eps) + dvar * 2 * (xT-mean) / m + dmean / m
