...
def compute_saliency_maps(X, y, model):
    ...
    score_max = torch.sum(scores.gather(1,y.view(-1,1)).squeeze())
...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(100):
      ...
      #extract the gradient of the scores wrt the input image
      im_grad = X_fooling.grad.data.abs()
      #update input image with normalized gradient
      X_fooling.data  += learning_rate * (im_grad/im_grad.norm()) 
      #reset the gradients to avoid gradient accumulation
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    im_grad -= 2*l2_reg*img.data  # Add l2 reg derivative.
