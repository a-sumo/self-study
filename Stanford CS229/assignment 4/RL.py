# -----------------------------------------------------------
# CS229: Machine Learning Assignment 4, EM, DL & RL
# 
# author: Armand Sumo

# email: armandsumo@gmail.com
#
# The following file is a modified version of control.py created by Stanford teacher Sanyam Mehra,
# the code that I wrote is in between places marked BEGIN YOUR CODE[...]END YOUR CODE.
# -----------------------------------------------------------
...
np.random.seed(0)
transition_probs = (1./NUM_STATES) * np.ones((NUM_STATES,NUM_ACTIONS,NUM_STATES))
value_func = np.random.random_sample(NUM_STATES)*0.1
state_rewards = np.zeros(NUM_STATES)
s_a_ns = np.zeros((NUM_STATES,NUM_ACTIONS,NUM_STATES))
new_state_reward = np.zeros(NUM_STATES)
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    action  = np.argmax(np.einsum('jk,k->j',transition_probs[state],value_func))
    ...
    s_a_ns[state,action,new_state] += 1
    # record the rewards for every `new_state`
    new_state_reward[new_state] = R
    ...
    if new_state == NUM_STATES - 1:
        ...
        # If a state-action pair has never been tried before, 
        # then no new_state has ever been associated to it  
        sp_sum = np.sum(s_a_ns,axis=2)
        valid_denom = np.where(np.sum(s_a_ns, axis=2) > 0)
        transition_probs[valid_denom] = s_a_ns[valid_denom]/np.reshape(sp_sum[valid_denom],(-1,1))

        pos_reward = np.where(reward_cnt>0)
        new_state_reward[pos_reward] = new_state_reward[pos_reward]/reward_cnt[pos_reward]
        ###### END YOUR CODE ######
        ...
        value_diff = np.zeros(NUM_STATES) + TOLERANCE + 1
        num_iter = 0
        while np.max(value_diff) > TOLERANCE:
            new_value_func = new_state_reward + GAMMA * np.max(np.matmul(transition_probs,value_func),axis=1)
            value_diff = np.abs(new_value_func - value_func)
